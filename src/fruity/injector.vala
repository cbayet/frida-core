namespace Frida.Fruity.Injector {
	...
	private class Session : Object {
		...
		private async void resolve_symbols (Gee.HashMap<string, Gee.HashMap<string, uint64?>> symbols, Cancellable? cancellable)
			...
			yield main_thread.write_register ("x0", input_vector_address, cancellable);
			yield main_thread.write_register ("x1", output_vector_address, cancellable);
			yield main_thread.write_register ("x2", dyld_fields.all_image_info, cancellable);
		...
		/* Inlined version of helpers/resolver.c */
