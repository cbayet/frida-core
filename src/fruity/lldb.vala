namespace Frida.LLDB {
	public class Client : Object, AsyncInitable {
		...
		private Exception? current_exception;
		...
		public async Process launch (string[] argv, LaunchOptions? options = null, Cancellable? cancellable = null) throws Error, IOError {
			if (options != null) {
			...
			register_by_name = yield get_register_mappings ();
		...
		private async Gee.HashMap<string, Register> get_register_mappings (Cancellable? cancellable = null)
				throws Error, IOError {
		...
		private void check_stopped () throws Error {
			if (state != STOPPED)
				throw new Error.INVALID_OPERATION ("Invalid operation when not STOPPED, current state is %s", state.to_string ());
		}
		...
		private void handle_stop (string data) throws Error {
			...
			uint64 signal_id;
			try {
				uint64.from_string (data[0:2], out signal_id, 16);
			...
			var exception = new Exception ((uint) signal_id, breakpoint, thread, context);
		...
		private class PropertyDictionary {
			...
			public uint get_uint (string name) throws Error {
				...
				try {
					...
					throw new Error.PROTOCOL ("Property '%s' is not a valid uint: %s", name, e.message);
				}
			...
			public Gee.ArrayList<uint> get_uint_array (string name) throws Error {
				...
				foreach (var element in get_string (name).split (",")) {
					uint64 val;
					try {
						uint64.from_string (element, out val, 16, uint.MIN, uint.MAX);
					} catch (NumberParserError e) {
						throw new Error.PROTOCOL ("Property '%s' is not a valid uint array: %s", name, e.message);
					}
					result.add ((uint) val);
				}
			...
			public Gee.ArrayList<uint64?> get_uint64_array (string name) throws Error {
				...
				foreach (var element in get_string (name).split (",")) {
					uint64 val;
					try {
						uint64.from_string (element, out val, 16);
					} catch (NumberParserError e) {
						throw new Error.PROTOCOL ("Property '%s' is not a valid uint64 array: %s", name, e.message);
					}
					result.add (val);
				}
	...
	public class Thread : Object {
		...
		public async Snapshot save_register_state (Cancellable? cancellable = null) throws Error, IOError {
			...
			return new Snapshot (Protocol.parse_uint (response.payload));
		}
	...
	public class Exception : Object {
		...
		public Exception (uint signal_id, Breakpoint? breakpoint, Thread thread, Gee.HashMap<string, uint64?> context) {
			Object (
				signal_id: signal_id,
	...
	namespace Protocol {
		internal uint64 parse_address (string raw_val) throws Error {
			...
			try {
				uint64.from_string (raw_val, out val, 16);
			} catch (NumberParserError e) {
		...
		internal uint parse_uint (string raw_val) throws Error {
			uint64 val;

			try {
				uint64.from_string (raw_val, out val, 10, uint.MIN, uint.MAX);
			} catch (NumberParserError e) {
				throw new Error.PROTOCOL ("Invalid response: %s", e.message);
			}
